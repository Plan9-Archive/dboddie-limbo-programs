...
adttest.b
Tests my intuition about references to ADTs, creating references and modifying
them.
...
channels.b
Reads strings and integers sequentially from spawned processes via two channels.

channel_words.b
Reads strings sequentially from a spawned process via a channel.

cmdlinedrawtest.b
Uses the Draw module to render an image which is exported as a PPM file in the
current directory.
...
drawtest.b
Draws an image on the display and saves the result to a PPM image, or to stdout
if no file name is passed as an argument.

drawtest-wm.b
Draws an image on the display and saves the result to a PPM image, or to stdout
if no file name is passed as an argument. Requires the window manager to be
running.
...
inherit.b
Implements a simple form of inheritance where an instance of a specialised ADT
contains an instance of the more general ADT it "inherits" from. Method calls
are forwarded to the base implementation before code in the specialised ADT is
executed.

low-level-windows.b
Shows how to create windows using the low-level API provided by the Screen ADT
in the Draw module.
...
newhashtest.b
Shows how the NewHash module can be used to store and retrieve associative data
without the overhead of the Hash module.
...
raisetest.b
Experimenting with defining and raising an exception, as well as declaring that
a function may contain code that raises an exception.
...
windowtest.b
Shows how to use the Wmclient module to open a window and respond to user input.
